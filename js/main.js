...
function showHeaderContextMenu(x, y) {
  const items = [
    ...
    { icon: '❓', label: 'מדריך ועזרה', shortcut: 'H', action: openGuide },
    { divider: true },
...
const DailyPlanner = (() => {
  ...
  const pruneDeletedTaskBlocks = () => {
    ...
    const taskIds = new Set((typeof tasks !== 'undefined' ? tasks : []).map(t => t.id));
    const initialLen = plannerBlocks.length;
  ...
  const pruneDeletedEventBlocks = () => {
    ...
    const eventIds = new Set((typeof events !== 'undefined' ? events : []).map(evt => evt.id));
    const initialLen = plannerBlocks.length;
...
if (window.DailyPlanner && typeof window.DailyPlanner.refreshEvents === 'function') {
  if (typeof eventsLoaded === 'undefined' || eventsLoaded || hasEventsCache) {
    window.DailyPlanner.refreshEvents();
  }
}

if (window.DailyPlanner && typeof window.DailyPlanner.startReminderTicker === 'function') {
  window.DailyPlanner.startReminderTicker();
}
