...
// VAPID public key for web push (legacy, kept for backward compatibility)
...
// FCM Configuration (same project as firebase-config.js)
const FCM_CONFIG = {
...
// ============================================
// FCM Token Management (New - Primary Method)
// ============================================
...
async function initFCMMessaging() {
    ...
    try {
        ...
        const app = initializeApp(FCM_CONFIG, 'fcm-notifications');
...
async function hasFCMToken() {
    return !!localStorage.getItem(FCM_LOCAL_TOKEN_KEY);
}
...
async function toggleNotificationsFromUser() {
    ...
    // Subscribe to VAPID (legacy) + FCM (new)
    const appServerKey = urlBase64ToUint8Array(PUSH_VAPID_PUBLIC_KEY);
    let sub = null;
    try {
        // VAPID subscription (kept for backward compatibility)
        sub = await reg.pushManager.subscribe({ userVisibleOnly: true, applicationServerKey: appServerKey });
        await savePushSubscriptionForUser(currentUser, sub);
        localStorage.setItem(PUSH_LOCAL_USER_KEY, currentUser);

        // FCM token (primary for Vercel serverless)
        const fcmToken = await getFCMToken();
        if (fcmToken) {
            await saveFCMTokenForUser(currentUser, fcmToken);
        }
