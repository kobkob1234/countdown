/**
 * Shared push-reminder core.
 *
 * Imported by BOTH entry points so the reminder logic can never drift:
 *   - api/cron.js                         (Vercel serverless / cron-job.org)
 *   - tools/push-sender/send-reminders.mjs (GitHub Actions)
 *
 * `db` (firebase-admin database) and `webpush` are INJECTED via initCore(),
 * so this module requires no third-party node_modules of its own and resolves
 * cleanly from either location. Each entry point does its own credential/VAPID
 * setup and calls initCore() before runCheck().
 */
const crypto = require('crypto');
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));

// Config read from env (identical env-var names in both environments).
...
// Injected by initCore() before runCheck() / cleanupOldPushSent() are called.
let db = null;
let webpush = null;
let APP_URL = normalizeAppUrl('http://localhost/');

function initCore({ db: database, webpush: wp, appUrl }) {
  db = database;
  webpush = wp;
...
module.exports = { initCore, runCheck, cleanupOldPushSent };
